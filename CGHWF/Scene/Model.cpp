...
/*
void Model::subdivideMesh(Mesh::Mesh& mesh)
{
	...
	std::unordered_map<std::pair<unsigned int, unsigned int>, unsigned int, pair_hash> edgeMap;
	...
	for (size_t i = 0; i < indices.size(); i += 3) {
		...
		unsigned int edge1 = edgeMap[{v0, v1}];
		unsigned int edge2 = edgeMap[{v1, v2}];
		unsigned int edge3 = edgeMap[{v2, v0}];

		if (edge1 == 0) {
			glm::vec3 edgeVertex = (vertices[v0] + vertices[v1]) / 2.0f;
			edge1 = newVertices.size();
			newVertices.push_back(edgeVertex);
			edgeMap[{v0, v1}] = edge1;
		}

		if (edge2 == 0) {
			glm::vec3 edgeVertex = (vertices[v1] + vertices[v2]) / 2.0f;
			edge2 = newVertices.size();
			newVertices.push_back(edgeVertex);
			edgeMap[{v1, v2}] = edge2;
		}

		if (edge3 == 0) {
			glm::vec3 edgeVertex = (vertices[v2] + vertices[v0]) / 2.0f;
			edge3 = newVertices.size();
			newVertices.push_back(edgeVertex);
			edgeMap[{v2, v0}] = edge3;
		}

		newIndices.push_back(v0);
		newIndices.push_back(edge1);
		newIndices.push_back(edge3);

		newIndices.push_back(v1);
		newIndices.push_back(edge2);
		newIndices.push_back(edge1);
